...
def compute_velocity_strains(
    ...
    """Compute strain rates directly from the spatial gradients of velocity."""
    r = int(strain_window)
    ...
    N      = correlate(cnt,  ones_k,  **kw)
    sum_x  = correlate(cnt,  x_kern,  **kw)
    sum_y  = correlate(cnt,  y_kern,  **kw)
    sum_x2 = correlate(cnt,  x2_kern, **kw)
    sum_y2 = correlate(cnt,  y2_kern, **kw)
    sum_xy = correlate(cnt,  xy_kern, **kw)

    sum_u  = correlate(u_z,  ones_k,  **kw)
    sum_v  = correlate(v_z,  ones_k,  **kw)
    sum_ux = correlate(u_z,  x_kern,  **kw)
    sum_uy = correlate(u_z,  y_kern,  **kw)
    sum_vx = correlate(v_z,  x_kern,  **kw)
    sum_vy = correlate(v_z,  y_kern,  **kw)
